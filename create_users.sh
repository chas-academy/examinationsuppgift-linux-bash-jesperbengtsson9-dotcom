#!/bin/bash
 ...
 # $0 är scriptets eget namn. Vi använder det i felmeddelandet tillsammans med <användarnamne1> och de
 ...
 for username in "$@"; do
         ...
         # --- Skapa welcome.txt med välkomstmeddelande och användarlista ---
         # Vi skapar filen welcome.txt i hemkatalogen. Med > skriver vi första raden,
         # ett personligt välkomstmeddelande till användaren. Om filen inte finns
         # skapas den automatiskt av >.
         ...
         # IFS=: talar om för 'read' att kolon ska användas som avdelare(separator),
         ...
         # så att man inte hälsar välkommen på sig själv

         while IFS=: read -r user_name password user_uid gid info home shell; do
           if [[ "$user_uid" -ge 1000 && "$user_uid" -ne 65534 && "$user_name" != "$user_name" ]]; then
                   echo "$user_name" >> "$welcome_file"
           fi
        done < /etc/passwd

        echo " welcome.txt har skapats med välkomstmeddelande och användarlista."
        ...
        # summan av läsa(4) + skriva(2) + exekvera(1).
        ...
        # 600 på en fil ger ägaren läs- och skrivrättigheter, ingen annan får något.
        ...
        # Kontot är skapat, mappstrukturen finns på plats, welcome.txt är skriven
        # och behörigheter är satta. Allt är klart för den här användaren.
